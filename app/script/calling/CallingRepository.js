...
z.calling.CallingRepository = class CallingRepository {
   ...
   */
  subscribe_to_events() {
    amplify.subscribe(z.event.WebApp.CALL.MEDIA.TOGGLE, () => this.switch_call_center(z.calling.enum.CALL_ACTION.TOGGLE_MEDIA, arguments).bind(this));
    amplify.subscribe(z.event.WebApp.CALL.STATE.DELETE, () => this.switch_call_center(z.calling.enum.CALL_ACTION.DELETE, arguments).bind(this));
    amplify.subscribe(z.event.WebApp.CALL.STATE.JOIN, this.join_call.bind(this));
    amplify.subscribe(z.event.WebApp.CALL.STATE.LEAVE, () => this.switch_call_center(z.calling.enum.CALL_ACTION.LEAVE, arguments).bind(this));
    amplify.subscribe(z.event.WebApp.CALL.STATE.REJECT, () => this.switch_call_center(z.calling.enum.CALL_ACTION.REJECT, arguments).bind(this));
    amplify.subscribe(z.event.WebApp.CALL.STATE.PARTICIPANT_LEFT, () => this.switch_call_center(z.calling.enum.CALL_ACTION.PARTICIPANT_LEFT, arguments).bind(this));
  ...
  join_call(conversation_id, video_send) {
    this.get_call_by_id(conversation_id)
      ...
      .then((call_state) => {
        if ((call_state === z.calling.enum.CallState.OUTGOING) && !z.calling.CallingRepository.supports_calling()) {
    ...
    return this.conversation_repository.get_conversation_by_id_async(conversation_id)
      ...
      .then((protocol_version) => {
        this.logger.log(`Selected outgoing call protocol version: ${protocol_version}`,
          {conversation_id: conversation_id, use_v3_api: this.use_v3_api});
      ...
      .then((protocol_version) => {
        switch (protocol_version) {
          case z.calling.enum.PROTOCOL.VERSION_2:
            return this.v2_call_center.state_handler[fn_name].apply(this, args);
          case z.calling.enum.PROTOCOL.VERSION_3:
            return this.v3_call_center[fn_name].apply(this, args);
          default:
   ...
   * @param {z.calling.enum.CallState} call_state - Call state of new call
    ...
    return new Promise((resolve) => {
      ...
      if (ongoing_call_id) {
        amplify.publish(z.event.WebApp.WARNING.MODAL, z.ViewModel.ModalType.CALL_START_ANOTHER, {
          ...
          close: function() {
            if (call_state === z.calling.enum.CallState.INCOMING) {
  ...
  _update_calling_config() {
    this.calling_service.get_config()
      .then((calling_config) => {
        ...
        this.calling_config_timeout = window.setTimeout(this._update_calling_config().bind(this), timeout_in_seconds * MILLISECONDS_IN_SECOND);
      });
  }
  ...
  report_call(conversation_id) {
    this.get_call_by_id(conversation_id)
      .catch(() => {
        for (const call_et of this.calls()) {
          if (!z.calling.enum.CallStateGroups.IS_ENDED.includes(call_et.state())) {
            return call_et;
          }
        }
      })
