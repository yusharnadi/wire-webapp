...
z.calling.mapper.SDPMapper = {
  ...
  map_e_call_message_to_object(e_call_message_et) {
    return Promise.resolve(new window.RTCSessionDescription({
      ...
      type: e_call_message_et.response === true ? z.calling.rtc.SDPType.ANSWER : z.calling.rtc.SDPType.OFFER,
    }));
  },
   ...
   * @param {z.calling.enum.SDPSource} [sdp_source=z.calling.enum.SDPSource.REMOTE] - Source of the SDP - local or remote
  ...
  rewrite_sdp(rtc_sdp, sdp_source = z.calling.enum.SDPSource.REMOTE, flow_et) {
    if (sdp_source === z.calling.enum.SDPSource.LOCAL) {
    ...
    rtc_sdp.sdp.split('\r\n').forEach(function(sdp_line) {
      ...
      // Remove once obsolete due to high uptake of clients based on AVS build 3.3.11 containing fix for AUDIO-1215
        ...
        if (sdp_source === z.calling.enum.SDPSource.REMOTE && z.util.Environment.browser.firefox && rtc_sdp.type === z.calling.rtc.SDPType.ANSWER) {
      ...
      // Code to nail in bit-rate and ptime settings for improved performance and experience
        ...
        if (flow_et.negotiation_mode() === z.calling.enum.SDP_NEGOTIATION_MODE.ICE_RESTART || (sdp_source === z.calling.enum.SDPSource.LOCAL && flow_et.is_group())) {
          sdp_lines.push(sdp_line);
