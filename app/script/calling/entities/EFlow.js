...
z.calling.entities.EFlow = class EFlow {
  ...
  constructor(e_call_et, e_participant_et, timings, e_call_message_et) {
    ...
    this.v3_call_center = this.e_call_et.v3_call_center;
    ...
    // Audio
    this.audio = new z.calling.entities.FlowAudio(this, this.e_call_et.media_repository.get_audio_context());
    ...
    this.connection_state = ko.observable(z.calling.rtc.ICEConnectionState.NEW);
    this.gathering_state = ko.observable(z.calling.rtc.ICEGatheringState.NEW);
    this.signaling_state = ko.observable(z.calling.rtc.SignalingState.NEW);

    this.connection_state.subscribe((ice_connection_state) => {
      switch (ice_connection_state) {
        case z.calling.rtc.ICEConnectionState.CHECKING:
          this.telemetry.schedule_check(this.e_call_et.telemetry.media_type);
          break;

        case z.calling.rtc.ICEConnectionState.COMPLETED:
        case z.calling.rtc.ICEConnectionState.CONNECTED:
          ...
          this.e_call_et.state(z.calling.enum.CallState.ONGOING);
          this.e_call_et.termination_reason = undefined;
          break;

        case z.calling.rtc.ICEConnectionState.CLOSED:
        ...
        case z.calling.rtc.ICEConnectionState.DISCONNECTED:
          this._set_negotiation_restart_timeout();
          break;

        case z.calling.rtc.ICEConnectionState.FAILED:
          if (this.e_call_et.self_client_joined()) {
    ...
    this.signaling_state.subscribe((signaling_state) => {
      switch (signaling_state) {
        case z.calling.rtc.SignalingState.CLOSED:
          this.logger.debug(`PeerConnection with '${this.remote_user.name()}' was closed`);
          this.e_call_et.delete_e_participant(this.e_participant_et.id, this.remote_client_id);
          this._remove_media_stream(this.media_stream());
          break;

        case z.calling.rtc.SignalingState.REMOTE_OFFER:
          this.negotiation_needed(true);
          break;

        case z.calling.rtc.SignalingState.STABLE:
          this._clear_negotiation_timeout();
          break;
    ...
    this.can_set_local_sdp = ko.pureComputed(() => {
      const in_connection_progress = this.connection_state() === z.calling.rtc.ICEConnectionState.CHECKING;
      const progress_gathering_states = [z.calling.rtc.ICEGatheringState.GATHERING, z.calling.rtc.ICEGatheringState.COMPLETE];
      const in_progress = in_connection_progress && progress_gathering_states.includes(this.gathering_state());

      const is_answer = this.local_sdp_type() === z.calling.rtc.SDPType.ANSWER;
      const is_offer = this.local_sdp_type() === z.calling.rtc.SDPType.OFFER;
      const in_remote_offer_state = this.signaling_state() === z.calling.rtc.SignalingState.REMOTE_OFFER;
      const in_stable_state = this.signaling_state() === z.calling.rtc.SignalingState.STABLE;
    ...
    this.can_set_remote_sdp = ko.pureComputed(() => {
      const is_answer = this.remote_sdp_type() === z.calling.rtc.SDPType.ANSWER;
      const is_offer = this.remote_sdp_type() === z.calling.rtc.SDPType.OFFER;
      const in_local_offer_state = this.signaling_state() === z.calling.rtc.SignalingState.LOCAL_OFFER;
      const in_stable_state = this.signaling_state() === z.calling.rtc.SignalingState.STABLE;
    ...
    this.can_create_sdp = ko.pureComputed(() => {
      const in_state_for_creation = this.negotiation_needed() && this.signaling_state() !== z.calling.rtc.SignalingState.CLOSED;
      return this.pc_initialized() && in_state_for_creation;
    });

    this.can_create_answer = ko.pureComputed(() => {
      const answer_state = this.is_answer() && this.signaling_state() === z.calling.rtc.SignalingState.REMOTE_OFFER;
      return this.can_create_sdp() && answer_state;
    });

    this.can_create_answer.subscribe((can_create) => {
      if (can_create) {
        this._create_answer();
      }
    });

    this.can_create_offer = ko.pureComputed(() => {
      const offer_state = !this.is_answer() && this.signaling_state() === z.calling.rtc.SignalingState.STABLE;
      return this.can_create_sdp() && offer_state;
    });

    this.can_create_offer.subscribe((can_create) => {
      if (can_create) {
        this._create_offer();
      }
    });
  ...
  restart_negotiation(negotiation_mode, is_answer, media_stream) {
    this.logger.debug(`Negotiation restart triggered by '${negotiation_mode}'`);
  ...
  _close_peer_connection() {
    if (this.peer_connection) {
      ...
      if (this.peer_connection.signalingState !== z.calling.rtc.SignalingState.CLOSED) {
        this.peer_connection.close();
        this.logger.debug(`Closing PeerConnection '${this.remote_user.name()}' successful`);
      }
    }
  }
  ...
  _create_peer_connection() {
    ...
    this.peer_connection.onaddstream = this._on_add_stream;
    this.peer_connection.ontrack = this._on_track;
    this.peer_connection.ondatachannel = this._on_data_channel;
    this.peer_connection.onicecandidate = this._on_ice_candidate;
    this.peer_connection.oniceconnectionstatechange = this._on_ice_connection_state_change;
    this.peer_connection.onremovestream = this._on_remove_stream;
    this.peer_connection.onsignalingstatechange = this._on_signaling_state_change;
  }
  ...
  _on_add_stream({stream: media_stream}) {
    this.logger.debug('Remote MediaStream added to PeerConnection',
      {
  ...
  _on_ice_candidate({candidate: ice_candidate}) {
    if (!ice_candidate) {
      if (this.should_send_local_sdp()) {
        this.logger.debug('Generation of ICE candidates completed - sending SDP');
  ...
  _on_ice_connection_state_change(event) {
    if (this.peer_connection || ![z.calling.enum.CallState.DISCONNECTING, z.calling.enum.CallState.ENDED].includes(this.e_call_et.state())) {
      this.logger.debug('State changed - ICE connection', event);
  ...
  */
  _on_remove_stream(event) {
    this.logger.debug('Remote MediaStream removed from PeerConnection', event);
  }
  ...
  _on_signaling_state_change(event) {
    this.logger.debug(`State changed - signaling state: ${this.peer_connection.signalingState}`, event);
    this.signaling_state(this.peer_connection.signalingState);
  }
   ...
   */
  _on_track(event) {
    this.logger.debug('Remote MediaStreamTrack added to PeerConnection', event);
  }
  ...
  send_message(e_call_message_et) {
    ...
    if (this.data_channel && this.e_call_et.data_channel_opened) {
      this.logger.debug(`Sending e-call '${type}' message to conversation '${this.e_call_et.conversation_et.id}' via data channel`, e_call_message_et.to_JSON());

      try {
        return this.data_channel.send(e_call_message_et.to_content_string());
      } catch (error) {
  ...
  _close_data_channel() {
    if (this.data_channel) {
      if (this.data_channel.readyState === z.calling.rtc.DataChannelState.OPEN) {
  ...
  _setup_data_channel(data_channel) {
    ...
    data_channel.onclose = this._on_close;
    data_channel.onerror = this._on_error;
    data_channel.onmessage = this._on_message;
    data_channel.onopen = this._on_open;
  }
  ...
  _on_close({target: data_channel}) {
    this.logger.debug(`Data channel '${data_channel.label}' was closed`, data_channel);

    if (this.data_channel && this.data_channel.readyState === z.calling.rtc.DataChannelState.CLOSED) {
      delete this.data_channel;
      this.e_call_et.data_channel_opened = false;
    }
  }
  ...
  _on_message({data: message}) {
    ...
    if (e_call_message.resp === true) {
      this.logger.info(`Received confirmation for e-call '${e_call_message.type}' message via data channel`, e_call_message);
    } else {
      this.logger.info(`Received e-call '${e_call_message.type}' message via data channel`, e_call_message);
    }

    amplify.publish(z.event.WebApp.CALL.EVENT_FROM_BACKEND, z.conversation.EventBuilder.build_calling(this.e_call_et.conversation_et, e_call_message, this.remote_user_id, this.remote_client_id));
  }
  ...
  _on_open({target: data_channel}) {
    this.logger.debug(`Data channel '${data_channel.label}' was opened and can be used`, data_channel);
  ...
  save_remote_sdp(e_call_message_et) {
    z.calling.mapper.SDPMapper.map_e_call_message_to_object(e_call_message_et)
      .then((rtc_sdp) => {
        return z.calling.mapper.SDPMapper.rewrite_sdp(rtc_sdp, z.calling.enum.SDPSource.REMOTE, this);
      })
      .then(({sdp: remote_sdp}) => {
        ...
        if (remote_sdp.type === z.calling.rtc.SDPType.OFFER) {
          switch (this.signaling_state()) {
            case z.calling.rtc.SignalingState.LOCAL_OFFER:
              if (this._solve_colliding_states()) {
                return;
              }
              break;
            case z.calling.rtc.SignalingState.NEW:
            case z.calling.rtc.SignalingState.STABLE:
              this.is_answer(true);
              break;
            default:
        ...
        this.logger.info(`Saved remote '${this.remote_sdp().type}' SDP`, this.remote_sdp());
      });
  }
  ...
  send_local_sdp(sending_on_timeout = false) {
    ...
    z.calling.mapper.SDPMapper.rewrite_sdp(this.peer_connection.localDescription, z.calling.enum.SDPSource.LOCAL, this)
      .then(({ice_candidates, sdp: local_sdp}) => {
        ...
        this.logger.info(`Sending local '${this.local_sdp().type}' SDP containing '${ice_candidates.length}' ICE candidates for flow with '${this.remote_user.name()}'\n${this.local_sdp().sdp}`);
        this.should_send_local_sdp(false);

        const response = this.local_sdp().type === z.calling.rtc.SDPType.ANSWER;
        ...
        return this.e_call_et.send_e_call_event(e_call_message_et)
          .then(() => {
            ...
            this.logger.info(`Sending local '${this.local_sdp().type}' SDP successful`, this.local_sdp());
          });
      })
      .catch((error) => {
        this.should_send_local_sdp(true);
  ...
  _create_answer() {
    ...
    this.logger.info(`Creating '${z.calling.rtc.SDPType.ANSWER}' for flow with '${this.remote_user.name()}'`);

    this.peer_connection.createAnswer()
      .then((sdp_answer) => {
        this.logger.debug(`Creating '${z.calling.rtc.SDPType.ANSWER}' successful`, sdp_answer);

        return z.calling.mapper.SDPMapper.rewrite_sdp(sdp_answer, z.calling.enum.SDPSource.LOCAL, this);
      })
      .then(({sdp: local_sdp}) => {
        this.local_sdp(local_sdp);
      })
      .catch((error) => {
        ...
        this.logger.error(`Creating '${z.calling.rtc.SDPType.ANSWER}' failed: ${name} - ${message}`, error);

        const attributes = {cause: name, step: 'create_sdp', type: z.calling.rtc.SDPType.ANSWER};
  ...
  _create_offer(restart) {
    ...
    this.logger.info(`Creating '${z.calling.rtc.SDPType.OFFER}' for flow with '${this.remote_user.name()}'`);

    this.peer_connection.createOffer(offer_options)
      .then((sdp_offer) => {
        this.logger.debug(`Creating '${z.calling.rtc.SDPType.OFFER}' successful`, sdp_offer);

        return z.calling.mapper.SDPMapper.rewrite_sdp(sdp_offer, z.calling.enum.SDPSource.LOCAL, this);
      })
      .then(({sdp: local_sdp}) => {
        this.local_sdp(local_sdp);
      })
      .catch((error) => {
        const {message, name} = error;
        this.logger.error(`Creating '${z.calling.rtc.SDPType.OFFER}' failed: ${name} - ${message}`, error);

        const attributes = {cause: name, step: 'create_sdp', type: z.calling.rtc.SDPType.OFFER};
        this.call_et.telemetry.track_event(z.tracking.EventName.CALLING.FAILED_RTC, undefined, attributes);
  ...
  _create_additional_payload() {
    ...
    return this.v3_call_center.create_payload_prop_sync(this.e_call_et.self_state.video_send(), false, $.extend({remote_user: this.remote_user, sdp: this.local_sdp().sdp}, payload));
  }
   ...
   */
  _set_local_sdp() {
    this.logger.info(`Setting local '${this.local_sdp().type}' SDP`, this.local_sdp());

    this.peer_connection.setLocalDescription(this.local_sdp())
      .then(() => {
        this.logger.debug(`Setting local '${this.local_sdp().type}' SDP successful`, this.peer_connection.localDescription);
      ...
      .catch((error) => {
        const {message, name} = error;
        this.logger.error(`Setting local '${this.local_sdp().type}' SDP failed: ${name} - ${message}`, error);

        const attributes = {cause: name, location: 'local', step: 'set_sdp', type: this.local_sdp() !== undefined ? this.local_sdp().type : undefined};
        this.call_et.telemetry.track_event(z.tracking.EventName.CALLING.FAILED_RTC, undefined, attributes);
   ...
   */
  _set_remote_sdp() {
    this.logger.info(`Setting remote '${this.remote_sdp().type}' SDP\n${this.remote_sdp().sdp}`, this.remote_sdp());

    this.peer_connection.setRemoteDescription(this.remote_sdp())
      .then(() => {
        this.logger.debug(`Setting remote '${this.remote_sdp().type}' SDP successful`, this.peer_connection.remoteDescription);
      ...
      .catch((error) => {
        const {message, name} = error;
        this.logger.error(`Setting remote '${this.remote_sdp().type}' SDP failed: ${name} - ${message}`, error);

        const attributes = {cause: name, location: 'remote', step: 'set_sdp', type: this.remote_sdp() !== undefined ? this.remote_sdp().type : undefined};
        this.call_et.telemetry.track_event(z.tracking.EventName.CALLING.FAILED_RTC, undefined, attributes);
    ...
    return this._replace_media_track(media_stream_info)
      .catch((error) => {
        ...
        if ([z.calling.v3.CallError.TYPE.NO_REPLACEABLE_TRACK, z.calling.v3.CallError.TYPE.RTP_SENDER_NOT_SUPPORTED].includes(type)) {
          this.logger.info(`Replacement of MediaStream and renegotiation necessary: ${message}`, error);
  ...
  _add_media_stream(media_stream) {
    ...
    if (this.peer_connection.addTrack) {
      return media_stream.getTracks()
        .forEach((media_stream_track) => {
          ...
          this.logger.info(`Added local '${media_stream_track.kind}' MediaStreamTrack to PeerConnection`,
            {
              audio_tracks: media_stream.getAudioTracks(),
              stream: media_stream,
              video_tracks: media_stream.getVideoTracks(),
    ...
    this.logger.info(`Added local '${media_stream.type}' MediaStream to PeerConnection`,
      {
        audio_tracks: media_stream.getAudioTracks(),
        stream: media_stream,
        video_tracks: media_stream.getVideoTracks(),
  ...
  _replace_media_stream(media_stream_info) {
    ...
    .then(() => {
      return this._remove_media_stream(this.media_stream());
    })
    .then(() => {
  ...
  _replace_media_track(media_stream_info) {
    ...
    return Promise.resolve()
      ...
      .then((rtp_sender) => {
        rtp_sender.replaceTrack(media_stream.getTracks()[0]);
      })
      .then(() => {
        this.logger.info(`Replaced the '${media_type}' track`);
  ...
  _remove_media_stream(media_stream) {
    if (this.peer_connection) {
      if (this.peer_connection.removeTrack) {
        if (this.peer_connection.signalingState === z.calling.rtc.SignalingState.STABLE) {
          return media_stream.getTracks()
            .forEach(({id: track_id, kind: track_type}) => {
              for (const rtp_sender of this.peer_connection.getSenders()) {
                const {track: media_stream_track} = {rtp_sender};

                if (media_stream_track.id === track_id) {
                  this.peer_connection.removeTrack(rtp_sender);
                  this.logger.info(`Removed local '${track_type}' MediaStreamTrack from PeerConnection`);
                  break;
                }
              }
            });
      ...
      if (this.peer_connection.signalingState !== z.calling.rtc.SignalingState.CLOSED) {
        ...
        this.logger.info(`Removed local '${media_stream.type}' MediaStream from PeerConnection`,
          {
            audio_tracks: media_stream.getAudioTracks(),
            stream: media_stream,
            video_tracks: media_stream.getVideoTracks(),
  ...
  reset_flow() {
    ...
    this.logger.info(`Resetting flow with user '${this.remote_user.id}'`);
    if (this.media_stream()) {
      this._remove_media_stream(this.media_stream());
    }
  ...
  _reset_signaling_states() {
    this.signaling_state(z.calling.rtc.SignalingState.NEW);
    this.connection_state(z.calling.rtc.ICEConnectionState.NEW);
    this.gathering_state(z.calling.rtc.ICEGatheringState.NEW);
  }
