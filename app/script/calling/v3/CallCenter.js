...
z.calling.v3.CallCenter = class CallCenter {
  ...
  on_event(event) {
    ...
    if (V3_CALL_CONFIG.SUPPORTED_EVENTS.includes(event_type)) {
      if (!V3_CALL_CONFIG.SUPPORTED_VERSIONS.includes(event_content.version)) {
  ...
  _on_cancel(e_call_message_et) {
    ...
    if (!response) {
      this.get_e_call_by_id(conversation_id)
        .then((e_call_et) => {
          e_call_et.verify_session_id(e_call_message_et);
        })
        .then((e_call_et) => {
          e_call_et.delete_e_participant(user_id, client_id, z.calling.enum.TERMINATION_REASON.OTHER_USER);
        })
        .then((e_call_et) => {
  ...
  _on_group_setup(e_call_message_et) {
    ...
    const {conversation_id, dest_client_id, dest_user_id} = e_call_message_et;
    ...
    this.get_e_call_by_id(conversation_id)
      .then((e_call_et) => {
        ...
        e_call_et.update_e_participant(e_call_message_et, e_call_message_et.response !== true);
      })
      .catch(function(error) {
        if (error.type !== z.calling.v3.CallError.TYPE.NOT_FOUND) {
          throw error;
  ...
  _on_group_start(e_call_message_et) {
    ...
    this.get_e_call_by_id(conversation_id)
      .then((e_call_et) => {
        if ([z.calling.enum.CallState.INCOMING, z.calling.enum.CallState.OUTGOING].includes(e_call_et.state())) {
  ...
  _on_prop_sync(e_call_message_et) {
    ...
    this.get_e_call_by_id(conversation_id)
      ...
      .then((e_call_et) => {
        this._confirm_e_call_message(e_call_et, e_call_message_et);
        e_call_et.update_e_participant(e_call_message_et);
      })
      .catch(function(error) {
        if (error.type !== z.calling.v3.CallError.TYPE.NOT_FOUND) {
          throw error;
      ...
      .then((e_call_et) => {
        ...
        if (response) {
          switch (e_call_et.state()) {
            case z.calling.enum.CallState.INCOMING:
            ...
            case z.calling.enum.CallState.OUTGOING:
    ...
    if (_.isBoolean(payload_type)) {
      ...
      switch (payload_type) {
        ...
        case z.media.MediaType.SCREEN:
          ...
          video_send_state = invert ? 'false' : this.self_state.video_send();
        ...
        case z.media.MediaType.VIDEO:
          screen_send_state = invert ? 'false' : this.self_state.screen_send();
  ...
  send_e_call_event(conversation_et, e_call_message_et) {
    ...
    this.get_e_call_by_id(conversation_id)
      .then((e_call_et) => {
        if ([z.calling.enum.E_CALL_MESSAGE_TYPE.HANGUP, z.calling.enum.E_CALL_MESSAGE_TYPE.PROP_SYNC].includes(type)) {
      ...
      .catch((error) => {
        if (![z.calling.v3.CallError.TYPE.NO_DATA_CHANNEL, z.calling.v3.CallError.TYPE.NOT_FOUND].includes(error.type)) {
          throw error;
        }
  ...
  _limit_message_recipients(e_call_message_et) {
    ...
    if (e_call_message_et.type === z.calling.enum.E_CALL_MESSAGE_TYPE.REJECT) {
      ...
      recipients_promise = this.user_repository.get_user_by_id(remote_user_id)
        .then((remote_user_et) => {
          return {
            remote_user_et: remote_user_et,
            self_user_et: this.user_repository.self(),
          };
        });
    }
  ...
  join_call(conversation_id, video_send = false) {
    this.get_e_call_by_id(conversation_id)
      .catch((error) => {
        ...
        return this._create_outgoing_e_call(z.calling.mapper.ECallMessageMapper.build_prop_sync(false, undefined, this.create_payload_prop_sync(video_send, false, {conversation_id})));
      })
      .then((e_call_et) => {
   ...
   * @param {z.calling.enum.TERMINATION_REASON} termination_reason - Optional on reason for call termination
   * @returns {undefined} No return value
   */
  leave_call(conversation_id, termination_reason) {
    this.get_e_call_by_id(conversation_id)
      .then((e_call_et) => {
        this.logger.debug(`Leaving e-call in conversation '${conversation_id}'`, e_call_et);
  ...
  participant_left(conversation_id, user_id) {
    this.get_e_call_by_id(conversation_id)
      .then((e_call_et) => {
        e_call_et.delete_e_participant(user_id, undefined, z.calling.enum.TERMINATION_REASON.MEMBER_LEAVE);
      })
      .then((e_call_et) => {
