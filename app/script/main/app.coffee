...
class z.main.App
  ...
  init_app: (is_reload = @_is_reload()) =>
    ...
    .catch (error) =>
      ...
      if is_reload and error.type not in [z.client.ClientError.TYPE.MISSING_ON_BACKEND, z.client.ClientError.TYPE.NO_LOCAL_CLIENT]
        @auth.client.execute_on_connectivity(z.service.BackendClient.CONNECTIVITY_CHECK_TRIGGER.APP_INIT_RELOAD).then -> window.location.reload false
      ...
      token_promise.catch (error) =>
        if is_reload
          if error.type in [z.auth.AccessTokenError.TYPE.REQUEST_FORBIDDEN, z.auth.AccessTokenError.TYPE.NOT_FOUND_IN_CACHE]
            ...
            @auth.client.execute_on_connectivity(z.service.BackendClient.CONNECTIVITY_CHECK_TRIGGER.ACCESS_TOKEN_RETRIEVAL).then -> window.location.reload false
        else if navigator.onLine
          switch error.type
  ...
  on_internet_connection_gained: =>
    ...
    @auth.client.execute_on_connectivity z.service.BackendClient.CONNECTIVITY_CHECK_TRIGGER.CONNECTION_REGAINED
    .then =>
  ...
  _redirect_to_login: (session_expired) ->
    ...
    @auth.client.execute_on_connectivity(z.service.BackendClient.CONNECTIVITY_CHECK_TRIGGER.LOGIN_REDIRECT)
    .then ->
