...
z.main.App = class App {
  ...
  _setup_utils() {
    return {
      debug: new z.util.DebugUtil(this.repository.user, this.repository.conversation),
    };
  }
   ...
   * @param {boolean} is_reload - Is initalization a page reload
  ...
  _watch_online_status() {
    ...
    $(window).on('offline', this.on_internet_connection_lost);
    $(window).on('online', this.on_internet_connection_gained);
  }
   ...
   */
  init_debugging() {
    if (z.util.Environment.frontend.is_localhost()) this._attach_live_reload();
  }
